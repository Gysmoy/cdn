...
class Tokenizer
{
    ...
    public static function encode($text)
    {
        ...
        foreach ($bpe_merges_temp as $bmt) {
            ...
            $split_bmt = array_filter($split_bmt, function ($n) {
                return Tokenizer::gpt_my_filter($n);
            });
            if (count($split_bmt) > 0) {
                $bpe_merges[] = $split_bmt;
            }
        }
        $bpe_ranks = Tokenizer::gpt_dictZip($bpe_merges, range(0, count($bpe_merges) - 1));

        $cache = array();
        foreach ($matches[0] as $token) {
            $new_tokens = array();
            $chars = array();
            $token = Tokenizer::gpt_utf8_encode($token);
            ...
            foreach ($chars as $char) {
                if (isset($byte_encoder[Tokenizer::gpt_unichr($char)])) {
                    $result_word .= $byte_encoder[Tokenizer::gpt_unichr($char)];
                }
            }
            $new_tokens_bpe = Tokenizer::gpt_bpe($result_word, $bpe_ranks, $cache);
    ...
    private static function gpt_bpe($token, $bpe_ranks, &$cache)
    {
        ...
        $word = Tokenizer::gpt_split($token);
        $init_len = count($word);
        $pairs = Tokenizer::gpt_get_pairs($word);
        if (!$pairs) {
            return $token;
        }
        while (true) {
            ...
            $i = 0;
            while ($i < count($word)) {
                $j = Tokenizer::gpt_indexOf($word, $first, $i);
                if ($j === -1) {
    ...
    public static function tokens(string $string): int
    {
